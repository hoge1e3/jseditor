  ...
  return Tonyu.klass.define({
    ...
    methods: {
      ...
      runMenuItem :function _trc_TMenu_runMenuItem(item) {
        ...
        //$LASTPOS=3000055;//jseditor.TMenu:55
        if (! item.action) {
          return _this;
        }
        //$LASTPOS=3000086;//jseditor.TMenu:86
        if (typeof  item.action=="string") {
          //$LASTPOS=3000131;//jseditor.TMenu:131
          a = item.action;
          
          //$LASTPOS=3000159;//jseditor.TMenu:159
          r = /(\$[\$_A-Za-z0-9]*)\.([\$_A-Za-z0-9]+)/.exec(a);
          
          //$LASTPOS=3000224;//jseditor.TMenu:224
          if (r) {
            //$LASTPOS=3000246;//jseditor.TMenu:246
            obj = Tonyu.globals[r[1]];
            
            //$LASTPOS=3000288;//jseditor.TMenu:288
            if (! obj) {
              throw new Error(r[1]+" not found");
              
            }
            //$LASTPOS=3000347;//jseditor.TMenu:347
            if (obj.parallel) {
              //$LASTPOS=3000365;//jseditor.TMenu:365
              obj.parallel(r[2]);
            } else {
              //$LASTPOS=3000403;//jseditor.TMenu:403
              obj[r[2]]();
            }
            
          } else {
            //$LASTPOS=3000447;//jseditor.TMenu:447
            _this.parallel(a);
            
          }
          
        } else {
          //$LASTPOS=3000486;//jseditor.TMenu:486
      ...
      make :function _trc_TMenu_make(title,hier) {
        ...
        //$LASTPOS=3000693;//jseditor.TMenu:693
        ul1 = UI("ul",{"class": "nav navbar-nav"});
        
        //$LASTPOS=3000744;//jseditor.TMenu:744
        _it_8=Tonyu.iterator(hier,1);
        while(_it_8.next()) {
          mainMenuItem=_it_8[0];
          
          //$LASTPOS=3000786;//jseditor.TMenu:786
          ...
          //$LASTPOS=3001098;//jseditor.TMenu:1098
          ul1.append(li);
          //$LASTPOS=3001123;//jseditor.TMenu:1123
          if (mainMenuItem.sub) {
            //$LASTPOS=3001160;//jseditor.TMenu:1160
            ul2 = UI("ul",{"class": "dropdown-menu"});
            
            //$LASTPOS=3001217;//jseditor.TMenu:1217
            mainMenuItem.sub.forEach((function anonymous_1242(subMenuItem) {
              
              //$LASTPOS=3001276;//jseditor.TMenu:1276
              if (subMenuItem.key) {
                //$LASTPOS=3001320;//jseditor.TMenu:1320
                KeyEventChecker.down(document,subMenuItem.key,(function anonymous_1367(e) {
                  
                  //$LASTPOS=3001399;//jseditor.TMenu:1399
                  _this.runMenuItem(subMenuItem);
                  //$LASTPOS=3001447;//jseditor.TMenu:1447
                  e.stopPropagation();
                  //$LASTPOS=3001490;//jseditor.TMenu:1490
                  e.preventDefault();
                  return false;
                }));
                
              }
              //$LASTPOS=3001606;//jseditor.TMenu:1606
              ul2.append(UI("li",["a",{id: subMenuItem.id||_this.subMeniItem.action,href: subMenuItem.href||"#",on: {click: (function anonymous_1842() {
                
                //$LASTPOS=3001845;//jseditor.TMenu:1845
                _this.runMenuItem(subMenuItem);
              })}},subMenuItem.label]));
            }));
            //$LASTPOS=3001993;//jseditor.TMenu:1993
            li.append(ul2);
            
          }
          
        }
        //$LASTPOS=3002032;//jseditor.TMenu:2032
        menu = UI("div",{"class": "collapse navbar-collapse"},ul1);
        
        //$LASTPOS=3002098;//jseditor.TMenu:2098
